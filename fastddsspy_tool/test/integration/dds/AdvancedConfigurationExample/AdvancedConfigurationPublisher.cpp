        ...
        unsigned int ownership_strength /* = 0 */)
{
    ...
    {
        // CREATE THE TOPIC
        topic_ = participant_->create_topic(topic_name+std::to_string(i), "HelloWorld", TOPIC_QOS_DEFAULT);
        ...
        writers_[i] = publisher_->create_datawriter(topic_, wqos, &listener_);
        ...
        std::cout << "Publisher Participant created with DataWriter Guid [ " << writers_[i]->guid() << " ]." << std::endl;
    }
    return true;
}
